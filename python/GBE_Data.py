...
def env_data(attribute, start_time=0, end_time=datetime.now().timestamp()):
...
def dew_point_data(start_time, end_time):
    print("Two Variable", start_time, end_time)
    group_name = GBE_ID
    ...
    for doc in recs:
        if TEMPERATURE in doc["attributes"] and HUMIDITY in doc["attributes"]:
            ...
            #print(data)
    return DataFrame({"time":time, "attribute":att, "value":value})
